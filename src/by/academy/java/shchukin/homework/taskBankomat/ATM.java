...
public class ATM {
	// делаем сет так как карты разные
			...
			if (!cashCards.contains(cashCardToCheck)) {
				...
				do {
					...
					if (moneyAmount % 10 == 0) {
						try {
							for (CashCard cashCard : cashCards) {
								if (cashCard.equals(cashCardToCheck)) {
									System.out.println("С вашего счета было снято " + cashCard.takeMoney(moneyAmount));

									System.out.println("На вашем счету осталось " + cashCard.getBalance());
									break;
								}
							}
						...
						System.out.println("Купюры данного номинала отсутствуют");
					}
				} while (true);
			}
		} while (true);
